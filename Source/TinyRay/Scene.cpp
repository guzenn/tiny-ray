...
{
	int i = 0;
	RayHitResult result = m_sceneObjects[i]->IntersectByRay(ray);

	for (i = 0; i < m_sceneObjects.size() - 1; i++)
	{
		RayHitResult newResult = m_sceneObjects[i + 1]->IntersectByRay(ray);
		if (newResult.t > 0 && newResult.t < result.t)
		{
			result = newResult;
		}
	}
